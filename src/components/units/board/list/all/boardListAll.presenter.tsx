import * as s from "./boardListAll.styles";
...
export default function BoardListAllUI(props: BoardListUIProps) {
  return (
    <s.Wrapper>
      {props.isPc && (
        <s.CategoryWrapper>
          <s.CategoryAll onClick={props.onClickMoveToAll}>All</s.CategoryAll>
          ...
          {props.data?.fetchBoards.map((el: any) => (
            <s.ProductInfoWrapper key={el.id}>
              ...
              <s.StatusPriceWrapper>
                ...
                {el.status === "거래완료" && <s.Status3>{el.status}</s.Status3>}
