...
export default function MainSlider(props: ICarouselProps) {
  ...
  return (
    <CustomSlider {...settings}>
      <s.AdCarousel
        src={`https://s3.brian-hong.tech/openrun/${props.Src[0]}`}
        color="#5358D6"
        onClick={() => onClickGoToList("/board/fashion/")}
      />
      <s.AdCarousel
        src={`https://s3.brian-hong.tech/openrun/${props.Src[1]}`}
        color="#fff"
        onClick={() => onClickGoToList("/board/toy/")}
      />
      <s.AdCarousel
        src={`https://s3.brian-hong.tech/openrun/${props.Src[2]}`}
        color="#111"
        onClick={() => onClickGoToList("/board/digital/")}
      />
      <s.AdCarousel
        src={`https://s3.brian-hong.tech/openrun/${props.Src[3]}`}
        color="#fff"
        onClick={() => onClickGoToList("/board/acc/")}
      />
      <s.AdCarousel
        src={`https://s3.brian-hong.tech/openrun/${props.Src[4]}`}
        color="#fff"
